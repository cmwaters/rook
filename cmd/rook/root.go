...
func newApp(logger log.Logger, db dbm.DB, traceStore io.Writer, appOpts servertypes.AppOptions) servertypes.Application {
	...
	return app.NewRookApp(logger, db, traceStore, true, skipUpgradeHeights,
...
func createWasnAppAndExport(
	...
	var RookApp *app.RookApp
	if height != -1 {
		RookApp = app.NewRookApp(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), app.GetEnabledProposals(), appOpts)

		if err := RookApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		RookApp = app.NewRookApp(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), app.GetEnabledProposals(), appOpts)
	}

	return RookApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
