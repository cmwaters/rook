...
// Setup initializes a new wasmd.RookApp. A Nop logger is set in RookApp.
func Setup(isCheckTx bool, homeDir string) *wasmd.RookApp {
	db := dbm.NewMemDB()
	app := wasmd.NewRookApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, homeDir, 5, wasm.EnableAllProposals, wasmd.EmptyAppOptions{})
	// app := wasmd.NewRookApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, 0)
	if !isCheckTx {
...
func SetupWithGenesisValSet(t *testing.T, homeDir string, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *wasmd.RookApp {
	db := dbm.NewMemDB()
	app := wasmd.NewRookApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, homeDir, 5, wasm.EnableAllProposals, wasmd.EmptyAppOptions{})
